...
//*************		Practica 12: 88 Millas por hora con Martin McFly******//
...
float angRot = 0.0;
float movKitX = 0.0;
float movKitY = 0.0;	//para el eje Y
float movKitZ = 0.0;
float rotKit = 0.0;
float rotKitX = 0.0;
float rotKitZ = 0.0;
...
float rotllantaI = 0.0;	//Rotar llantas
float rotllantaD = 0.0;
...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	glRotatef(rotllantaD, 0, 0, -1); //rotacion de las llantas cuando vuela
	...
	glTranslatef(6, -1, 7.5);
	glRotatef(rotllantaI, 0, 0, 1); //rotacion de las llantas cuando vuela
	glRotatef(180, 0, 1, 0);
	glRotatef(rotTires, 1, 0, 0);
	llanta.GLrender(NULL, _SHADED, 1.0);
	glPopMatrix();
	...
	glTranslatef(6, -1, -9.5);
	glRotatef(rotllantaI, 0, 0, 1); //rotacion de llanta cuando vuela
	glRotatef(180, 0, 1, 0);
	glRotatef(rotTires, 1, 0, 0);
	llanta.GLrender(NULL, _SHADED, 1.0);
	glPopMatrix();
	...
	if (circuito)
	{
		...
		if (recorrido8) //h
		{
			...
			if (movKitZ >= 150) {
				...
				movKitY = 0.0; // recorrido 8 completo e inicamos el 2
				recorrido8 = false;
				recorrido2 = true;
			}
		}
...
int main(int argc, char** argv)   // Main Function
{
	...
	glutCreateWindow("88 Millas por hora con Martin McFly"); // Nombre de la Ventana
	...
	glutCreateMenu(menu);
	glutAddSubMenu("88 Millas por hora con Martin McFly", submenu);
